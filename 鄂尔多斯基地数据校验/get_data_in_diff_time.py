...
# 定义输入和输出文件路径
input_file_path = r".\采集需求_EE.xlsx"
output_dir = r"."
...
# 定义接口地址
api_url = "http://10.100.59.1:8081/japrojecttag/timeseries"
...
for timestamp, group in grouped:
    ...
    try:
        ...
        # 将 timestamp 转换为标准格式
        timestamp_dt = datetime.strptime(timestamp.strip(),
                                         "%Y-%m-%d %H:%M:%S.%f" if "." in timestamp else "%Y/%m/%d %H:%M")
        formatted_timestamp = timestamp_dt.strftime("%Y/%m/%d %H:%M")
    except ValueError as e:
        print(f"时间戳格式错误，跳过该时间戳：{timestamp}，错误信息：{e}")
        continue
    ...
    for item in response_data["data"]:
        ...
        # 找到离目标时间戳最近的数据
        nearest_data = find_nearest_data(time_series, formatted_timestamp)
        if nearest_data:
            results.append({
                "采集点编码": tag_code,
